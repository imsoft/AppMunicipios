...
const SubsectionPage = () => {
  ...
  useEffect(() => {
    if (subsection) {
      const pathString = Array.isArray(subsection)
        ? subsection.join("/")
        : subsection;
  ...
  const buildLink = (name: string) => {
    const basePath = Array.isArray(subsection)
      ? subsection.join("/")
      : subsection;
  ...
  const currentPage = subsection
    ? Array.isArray(subsection)
      ? decodeURIComponent(subsection[subsection.length - 1])
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mx-auto max-w-2xl lg:mx-0">
          ...
          <p className="mt-6 text-lg leading-8 text-gray-600">
            Aquí va el contenido del directorio donde se explica el tema de
            transparencia.
          </p>
        </div>
      </div>
                  ...
                  <div className="mx-auto flex max-w-6xl justify-between gap-x-6">
                    <div className="flex min-w-0 gap-x-4">
                      <div className="mb-6 flex h-10 w-10 items-center justify-center rounded-lg bg-gray-50">
                        <folderIcon.icon
                          className={`${folderIcon.color} h-6 w-6`}
                          aria-hidden="true"
                        />
                      </div>
